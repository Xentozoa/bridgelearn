...
const dsaProblems = [
  {
    ...
    difficulty: "Easy",
    expected_output: "olleh" // Sample test case
  },
  {
    id: 2,
...
const DSAChallenge = ({ db, auth, myPoints, setMyPoints }) => {
  ...
  const judge0ApiKey = "YOUR_JUDGE0_API_KEY"; // Replace with your Judge0 API key

  useEffect(() => {
    if (!db || !userId) return;
    if (status === "idle") return; // Only listen when not idle
    ...
    return () => unsubscribe();
  }, [db, userId, myPoints, setMyPoints, status]); // status is now a dependency
  ...
  useEffect(() => {
      if (status === "in-progress" && timer === 0 && !solutionSubmitted) {
          submitSolution(false);
      }
  }, [timer, status, solutionSubmitted]);
  ...
  const submitSolution = async () => {
    if (status !== "in-progress" || !challenge || solutionSubmitted) return;
    setLoading(true);

    if (!judge0ApiKey || judge0ApiKey === "YOUR_JUDGE0_API_KEY") {
    ...
    try {
      ...
      const isSolutionCorrect = result.status.id === 3;
      
      await updateDoc(doc(db, "dsaChallenges", challenge.id), {
          status: "completed",
          winnerId: isSolutionCorrect ? userId : null,
    ...
    <div className="box-card dsa-challenge-card">
      ...
      {status === "idle" && (
        <div style={{ textAlign: 'center' }}>
          ...
          <p style={{marginTop: '1rem', color: 'var(--accent-1)'}}>Click the button to start searching for an opponent!</p>
          <button className="link" onClick={findOrCreateChallenge} disabled={loading} style={{maxWidth: '250px'}}>
      ...
      {status === "in-progress" && challenge && (
        <div style={{ textAlign: 'center' }}>
          <div className="battle-header">
             ...
             <div className="timer-box">Time Left: {Math.floor(timer / 60)}:{('0' + (timer % 60)).slice(-2)}</div>
          </div>
          ...
          <div className="code-options">
            <select className="language-select" value={language} onChange={(e) => setLanguage(e.target.value)} disabled={true}>
          ...
          <Editor
            ...
            theme="vs-dark"
            options={{
                domReadOnly: solutionSubmitted,
                readOnly: solutionSubmitted,
            }}
          />
          <button className="link" onClick={submitSolution} disabled={loading || solutionSubmitted} style={{ marginTop: '1rem' }}>
